...
parser = argparse.ArgumentParser(description='official Implementation of GoMVSNet')
parser.add_argument('--mode', default='train', help='train or test', choices=['train', 'test', 'profile'])
parser.add_argument('--model', default='mvsnet', help='select model')
parser.add_argument('--device', default='cuda', help='select model')
parser.add_argument('--dataset', default='dtu_yao', help='select dataset')
parser.add_argument('--trainpath', help='train datapath')
parser.add_argument('--normalpath', help='normal path')
...
if __name__ == '__main__':
    ...
    # model, optimizer
    model = SFMVSNEt(ndepths=[int(nd) for nd in args.ndepths.split(",") if nd],
    ...
    if is_distributed:
        print("Distributed mode, let's use", torch.cuda.device_count(), "GPUs!")
        model = nn.SyncBatchNorm.convert_sync_batchnorm(model)
        print("model sync success, local_rank=",args.local_rank)
        model = torch.nn.parallel.DistributedDataParallel(
            model, device_ids=[args.local_rank], output_device=args.local_rank
        )
        print("model distributed success")
    else:
    ...
    train_dataset = MVSDataset(args.trainpath, args.normalpath, args.trainlist, "train", args.nviews, args.numdepth, args.interval_scale)
    test_dataset = MVSDataset(args.testpath, args.normalpath, args.testlist, "test", args.nviews, args.numdepth, args.interval_scale)
