#!/usr/bin/env bash
MVS_TRAINING="/root/autodl-tmp/blendermvs/"  # path to BlendedMVS dataset
CKPT="xoutputs/dtu_training/model_000012.ckpt" # path to checkpoint
NORMAL_PATH="/root/autodl-tmp/highresblend/"
LOG_DIR="/root/autodl-tmp/outputs/bld_finetune"
if [ ! -d $LOG_DIR ]; then
	mkdir -p $LOG_DIR
fi
