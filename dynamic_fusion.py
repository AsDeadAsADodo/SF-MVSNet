...
def filter_depth(scan_folder, out_folder, pair_path, plyfilename, photo_threshold):
    ...
    for ref_view, src_views in pair_data:
        ...
        confidence = read_pfm(os.path.join(scan_folder, 'confidence/{:0>8}_3.pfm'.format(ref_view)))[0]
        ...
        photo_mask0 = confidence > photo_threshold[2] # stage3
...
def worker(scan):
    ...
    if (args.test_dataset=='tnt'):
        ...
        scans_iter = ['Family','Francis','Horse','Lighthouse','M60','Panther','Playground','Train']
        scans_adv = ['Auditorium','Ballroom','Courtroom','Museum','Palace','Temple']
        # scans_adv = ['Auditorium']
        
        scans_all = scans_iter + scans_adv
        # set threshold
        pc_mask_thres = {'Family':(0.5, 0.5, 0.5),
        'Francis':(0.5, 0.8, 0.5),
        'Horse':(0.4, 0.3, 0.2),
        'Lighthouse':(0.6, 0.7, 0.6),
        'M60':(0.9, 0.7, 0.3),
        'Panther':(0.5, 0.9, 0.3),
        'Playground':(0., 0.9, 0.),
        'Train':(0.7, 0.6, 0.6),
        ...
        if scan in scans_all:
            photo_threshold = pc_mask_thres["{}".format(scan)]
            print("photo_threshold of{} :{}".format(scan,photo_threshold))
